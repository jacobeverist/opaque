...
class ContactReferences:
	...
	def __init__(self, probe):
		...
		for i in range(0,self.numJoints):
			...
			size = ogre.Vector3(0.075,0.075,0.075)
			node.setScale(size)
	...
	def step(self):
		...
		for i in range(self.numJoints):
			...
			if self.activeRef[i]:
				pos = self.activeRefPtr[i].getRefPose()
				position = ogre.Vector3(pos[0],0.1,pos[1])
	...
	def getPoseProfile(self):
		...
		sadPoints = [4, 8, 12, 16, 20, 24, 28, 32, 36]
		infPoints = [2, 6, 10, 14, 18, 22, 26, 30, 34]
		...
		for i in range(9):
			j = infPoints[i]
			pose = self.getClosestPose(j)
			inflectionNodes.append(pose)
		...
		for i in range(9):
			...
			if i % 2 == 0 :
				L_saddleNodes.append(pose)
			else:
				R_saddleNodes.append(pose)
		
		refPoint = self.getClosestPose(19)

		pose = PoseProfile(L_saddleNodes, R_saddleNodes, inflectionNodes, refPoint)
	
		return pose

	# capture the wall reference points for later pose offset correction
	# later I build the wall contours from these points and use them in error correction
	def captureRef(self):

		sadPoints = [4, 8, 12, 16, 20, 24, 28, 32, 36]
		infPoints = [2, 6, 10, 14, 18, 22, 26, 30, 34]

		# 9     saddle points: 4,8,12,16,20,24,28,32,36
		# 9 inflection points: 2,6,10,14,18,22,26,30,34

		self.infNodes.write(str(self.poseCount) + " ")
		self.infNodesG.write(str(self.poseCount) + " ")
		self.sadNodes.write(str(self.poseCount) + " ")
		self.sadNodesG.write(str(self.poseCount) + " ")
		...
		# write the points to our capture files
		# this is the inflectio points that make the centerline of the sinusoidal pose
		for i in range(9):
			j = infPoints[i]

			if self.activeRef[j]:
				nodeID = self.activeRefPtr[j].getNodeID()

				x, z, p = self.activeRefPtr[j].getRefPose()
				self.infNodes.write(str(nodeID) + " " + str(x) + " " + str(z) + " ")

				inflectionNodes.append([x, z, nodeID])

				x, z, p = self.activeRefPtr[j].getGroundTruthPose()
				self.infNodesG.write(str(nodeID) + " " + str(x) + " " + str(z) + " ")

			else :
				self.infNodes.write("-1 0 0 ")
				self.infNodesG.write("-1 0 0 ")

		# this is the contours of the wall, contact points to the walls
		for i in range(9):
			j = sadPoints[i]
			...
			if self.activeRef[j]:
				...
				x,z,p = self.activeRefPtr[j].getRefPose()
				self.sadNodes.write(str(nodeID) + " " + str(x) + " " + str(z) + " ")

				if i % 2 == 0 :
					L_saddleNodes.append([x, z, nodeID])
				else:
					R_saddleNodes.append([x, z, nodeID])

				x,z,p = self.activeRefPtr[j].getGroundTruthPose()
				self.sadNodesG.write(str(nodeID) + " " + str(x) + " " + str(z) + " ")

			else:
				self.sadNodes.write("-1 0 0 ")
				self.sadNodesG.write("-1 0 0 ")

		self.infNodes.write("\n")
		self.infNodesG.write("\n")
		self.sadNodes.write("\n")
		self.sadNodesG.write("\n")

		# 1. grab the new pose
		# 2. perform a correction with the previous pose
		# 3. add the corrected pose to a new list of poses
		# 4. create the new path spline

		self.L_saddlePoses.append(L_saddleNodes) 
		self.R_saddlePoses.append(R_saddleNodes)
		self.inflectionPoses.append(inflectionNodes)


		nodeID = self.activeRefPtr[19].getNodeID()
