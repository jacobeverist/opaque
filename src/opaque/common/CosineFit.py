...
class CosineFit:
	...
	def findClosestPoint(self, point):
		...
		if point[0] <= p1[0]:
			min1 = sqrt((point[0]-p1[0])**2 + (point[1]-p1[1])**2)
			curvePoint1 = p1

		elif point[0] > p1[0] and point[0] < p2[0]:
			min1 = fabs(point[1])
			curvePoint1 = [point[0],0.0]

		elif point[0] >= p2[0]:
			min1 = sqrt((point[0]-p2[0])**2 + (point[1]-p2[1])**2)
			curvePoint1 = p2
		else:
		...
		elif point[0] > p3[0] and point[0] < p4[0]:
			min2 = fabs(point[1])
			curvePoint2 = [point[0],0.0]

		elif point[0] >= p4[0]:
			min2 = sqrt((point[0]-p4[0])**2 + (point[1]-p4[1])**2)
			curvePoint2 = p4
		...
		for x_samp in samples:
			...
			dist = sqrt((point[0]-(self.lead_len + x_samp))**2 + (point[1]-z)**2)
