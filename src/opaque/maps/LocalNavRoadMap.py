...
class LocalNavRoadMap(Map):
	...
	def __init__(self, localNode):
		...
		#self.roadGraph = self.localNode.voronoiMap.getGraph()
		self.roadGraph = 0
		self.probe = self.localNode.probe
		
		self.fileName = "localNavRoadMap%04u.png"
		...
		self.nodeCount = self.localNode.getNodeID()
		self.childNodes = []
		self.childEntities = []
		#self.parentNode = self.probe._mgr.getRootSceneNode().createChildSceneNode("localNavRoadCurveRoot" + str(self.nodeCount))
	...
	def draw(self):
		...
		for i in range(len(self.cleanPath)):
			...
			childNode = self.parentNode.createChildSceneNode("localNavRoadCurvePoint" + str(self.nodeCount) + "_" + str(i))
			self.childNodes.append(childNode)
	
			currEntity = self.probe._mgr.createEntity("localNavRoadCurveEnt" + str(self.nodeCount) + "_" +str(i), "Cube.mesh")
