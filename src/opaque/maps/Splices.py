...
def __remote_multiFit(rank, qin, qout, globalPath, medial, initPose, pathID, nodeID):
    ...
    while 1:
        ...
        results = []
        for arg in args:
            results.append(multiFitSplice(arg[0], globalPath, medial, initPose, pathID, nodeID, arg[1]))
...
def multiFitSplice(initGuess, orientedPath, medialAxis, initPose, pathID, nodeID, pathPlotCount = 0):
    ...
    resultPose, lastCost, matchCount = globalOverlapICP_GPU2([u1,u2,angGuess], orientedPath, medialAxis, globalPlotCount = pathPlotCount, plotIter = True, n1 = nodeID, n2 = -1)
        
    #self.nodeHash[nodeID].setGPACPose(resultPose)
    
    #pathPlotCount = 0
    resultArgs = getMultiDeparturePoint(orientedPath, medialAxis, resultPose, pathID, nodeID, pathPlotCount, plotIter = True)
...
def batchGlobalMultiFit(initGuesses, globalPath, medial, initPose, pathID, nodeID):
    ...
    pool = [processing.Process(target=__remote_multiFit,
                args=(rank, qin, qout, globalPath, medial, initPose, pathID, nodeID))
...
def getMultiDeparturePoint(currPath, medial2, estPose2, pathID, nodeID, pathPlotCount, plotIter = False):
    ...
    if len(currPath) == 0:
        return departurePoint1, angle1, isInterior1, isExist1, 0.0, departurePoint2, angle2, isInterior2, isExist2, 0.0, 0.0, 0.0
    ...
    if plotIter:
        ...
        pylab.savefig("distances_%04u.png" % pathPlotCount)
    ...
    if plotIter:
        ...
        pylab.savefig("departure_%04u.png" % pathPlotCount)
