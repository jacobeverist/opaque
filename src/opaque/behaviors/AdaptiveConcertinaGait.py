...
class AdaptiveConcertinaGait(Behavior):
	...
	def drawFit(self):
		...
		xP = []
		yP = []
		for p in cmdSegPoints:
			xP.append(p[0])
			yP.append(p[1])		
		pylab.plot(xP,yP,color='0.0')
		"""

		" highlight the joints of the current active peak "
		"""
		for i in peakJoints:
			xP = [actSegPoints[i+1][0]]
			yP = [actSegPoints[i+1][1]]
			pylab.scatter(xP,yP,linewidth=4, color=(1.0,0.8,1.0))
			xP = [cmdSegPoints[i+1][0]]
			yP = [cmdSegPoints[i+1][1]]
			pylab.scatter(xP,yP,linewidth=4, color=(1.0,0.8,1.0))

		for i in range(1, len(cmdSegPoints)-2):
			xP = [cmdSegPoints[i][0]]
			yP = [cmdSegPoints[i][1]]
			val = errors[i-1]
			if val > 1.0:
	...
	def step(self):
		...
		if self.transDone:
			...
			if not isDone:
				...
				"""
				Width Increasing Anchoring Method
				
				1. amplify peak
				2. if amplification doesn't cause error
				3. increase width up to a maximum
				4. if the width causes error, find the fit
				5. otherwise, put width back to nominal
				6. goto 1
				...
				if False:
					...
					if maxError > 0.3 and self.maxAmp != 0.0:					
						" perform widening operation here "
						" narrow down the width position "

						" if the difference between widths is < 0.01 "
						if fabs(self.maxWidth-self.minWidth) < 0.01:
							...
							if currWidth == self.minWidth:
								
								" C:  Overshoot.  Minimum is too high." 
								" Reduce amplitude minimum, set current to maximum"
								
								self.minWidth -= self.widthInc
								self.maxWidth = currWidth
								nextWidth = self.minWidth

								" we're making the width smaller than it started out "
								if self.minWidth < self.nomWidth:
									" reset to nominal width and go back to amplitude "
									self.isWidthMode = False
									self.widthLeft = True
									self.maxWidth = self.nomWidth
									self.minWidth = self.nomWidth
						...
						if currWidth > self.widthThresh:
							...
							self.minWidth = currWidth
		
							nextWidth = self.minWidth + self.widthInc
							
							" maximum is the value we just set "
							self.maxWidth = nextWidth
					...
					if maxError > 0.3 and self.maxAmp != 0.0:
						...
						#if self.currPeak != 0 and currAmp > 0.30 and not self.widthLeft and self.localFit.getPeakJoints(self.currPeak).count(38) == 0 and self.localFit.getPeakJoints(self.currPeak+1).count(38) == 0:
						#if self.currPeak != 0 and currAmp > 0.60 and not self.widthLeft and self.localFit.getPeakJoints(self.currPeak).count(38) == 0 and self.localFit.getPeakJoints(self.currPeak+1).count(38) == 0:
						if False:
							" now we go to width increasing mode, so set the flag "
							self.isWidthMode = True
							nextWidth = currWidth + self.widthInc
							self.maxWidth = nextWidth
				...
				" increase amplitude of the current peak "
				#val = self.curve.getPeakAmp(self.currPeak)
				#val += 0.04
				...
				if self.currPeak == 0:					
					...
					#self.curve.setPeakWidth(self.currPeak, nextWidth)
					#self.curve.setPeakWidth(self.currPeak+1, nextWidth)
