...
class PathCurveFit(Behavior):
	...
	def __init__(self, probe, mapGraph, curve = 0):
		...
		self.childNodes = []
		self.childEntities = []
		self.parentNode = self.probe._mgr.getRootSceneNode().createChildSceneNode("pathCurveRoot" + str(self.nodeCount))
		...
		position = ogre.Vector3(0.0,-10.0,0.0)
	...
	def draw(self):
		...
		for i in arange(0.0,1.0, 0.01):
			...
			transVec = dot(transpose(self.localNode.R), finalVec)
			resVec = dot(self.localNode.backR, transVec)
			resVec[0, 0] += self.localNode.dist
			tempVec = dot(self.localNode.foreR, resVec)
			pnt[0] = tempVec[0, 0]
			pnt[1] = tempVec[1, 0]
			...
			childNode = self.parentNode.createChildSceneNode("pathCurvePoint" + str(self.nodeCount) + "_" + str(i))
			self.childNodes.append(childNode)
	
			currEntity = self.probe._mgr.createEntity("pathCurveEnt" + str(self.nodeCount) + "_" +str(i), "Cube.mesh")
	...
	def step(self):
		...
		origin = self.getSegPose(segments[0])
		xTotal = origin[0]
		yTotal = origin[1]
		totalAngle = origin[2]
	...
	def old_step(self):
		...
		#origin = self.probe.getActualJointPose(self.startNode)
		origin = self.getJointPose(self.startNode)
	...
	def computePathDirection(self):
		...
		for i in joints:
			...
			poseVec = [cos(totalAngle),sin(totalAngle)] 
			vecSum1[0] += poseVec[0]
			vecSum1[1] += poseVec[1]
			dist, linePoint, orientVec = self.curve.findClosestPoint([xTotal, zTotal])
