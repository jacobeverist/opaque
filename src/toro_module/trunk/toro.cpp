...
int runAlg(int nPoints, int nEdges, int *vertIDs, double *vertPoses, int *vertPairs, double *offsets, double *covars) {
	TreeOptimizer2 pg;
	int  iterations=100;
	...
	//pg.load();
	pg.loadDirect(nPoints, nEdges, vertIDs, vertPoses, vertPairs, offsets, covars);


	// zero out the input arrays so we don't accidentally carry over the input data to the output
	...
	pg.readDirect(nPoints, nEdges, vertIDs, vertPoses, vertPairs, offsets, covars);
