...
bool TreePoseGraph2::loadDirect(int nPoints, int nEdges, int *vertIDs, double *vertPoses, int *vertPairs, double *offsets, double *covars){
	...
	for ( int i = 0 ; i < nEdges ; i++ ) {
		...
		p.x() = offsets[3*i];
		p.y() = offsets[3*i+1];
		p.theta() = offsets[3*i+2];

		m.values[0][0] = covars[6*i];
		m.values[0][1] = covars[6*i+1];
		m.values[1][1] = covars[6*i+2];
		m.values[2][2] = covars[6*i+3];
		m.values[0][2] = covars[6*i+4];
		m.values[1][2] = covars[6*i+5];
...
bool TreePoseGraph2::readDirect(int nPoints, int nEdges, int *vertIDs, double *vertPoses, int *vertPairs, double *offsets, double *covars) {
	...
	for (TreePoseGraph2::VertexMap::const_iterator it=vertices.begin(); it!=vertices.end(); it++){
		...
		vertIDs[i] = v->id;
		vertPoses[3*i] = v->pose.x();
		vertPoses[3*i+1] = v->pose.y();
		vertPoses[3*i+2] = v->pose.theta();
	...
	for (TreePoseGraph2::EdgeMap::const_iterator it=edges.begin(); it!=edges.end(); it++){
		...
		vertPairs[2*i] = e->v1->id;
		vertPairs[2*i+1] = e->v2->id;
		
		Pose p=e->transformation.toPoseType();

		offsets[3*i+0] = p.x();
		offsets[3*i+1] = p.y();
		offsets[3*i+2] = p.theta();

		covars[6*i+0] = e->informationMatrix.values[0][0];
		covars[6*i+1] = e->informationMatrix.values[0][1];
		covars[6*i+2] = e->informationMatrix.values[1][1];
		covars[6*i+3] = e->informationMatrix.values[2][2];
		covars[6*i+4] = e->informationMatrix.values[0][2];
		covars[6*i+5] = e->informationMatrix.values[1][2];
