...
template <class Ops>
typename TreePoseGraph<Ops>::Vertex* TreePoseGraph<Ops>::addVertex(int id, const Pose& pose){
  Vertex* v=vertex(id);
  if (v)
    return 0;
  v=new Vertex;
...
template <class Ops>
typename TreePoseGraph<Ops>::Edge* TreePoseGraph<Ops>::addEdge(Vertex* v1, Vertex* v2, 
								 const Transformation& t, const Information& i){
  if (v1==v2)
    return 0;
...
template <class Ops>
typename TreePoseGraph<Ops>::Edge* TreePoseGraph<Ops>::removeEdge(Edge* e){
  {
...
bool TreePoseGraph<Ops>::buildSimpleTree(){
  ...
  for (typename VertexMap::iterator it=vertices.begin(); it!=vertices.end(); it++){
    ...
    if ( root == 0 )
	    printf("found = %d, v = %d, root = %d, bestId = %d\n", found, v->id, root, bestId);
    else 
	    printf("pfound = %d, v = %d, root = %d, bestId = %d\n", found, v->id, root->id, bestId);
    if (found){
      ...
      if ( root == 0 ) 
		printf("root = %d, bestId = %d\n", root, bestId);
	  else
		printf("proot = %d, bestId = %d\n", root->id, bestId);
