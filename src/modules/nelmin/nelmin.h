...
void nelmin ( int n, double start[], double xmin[], 
	      ...
	      double angNom, double angLim, double uHigh, double uLow
	      )
{
  ...
  *numres = 0;
  ...
  for ( ; ; ) {
    ...
    y[n] = objFunc(start, d_matchPairs, d_offset, d_sum, numPairs, pose1, poses_2, numPoses, angNom, angLim, uHigh, uLow, offset); 
	//printf("y[%d] = %lf\n", n, y[n]);
    ...
    *icount = *icount + 1;
	...
	p[i+j*n] = start[i];
      ...
      //y[j] = (*fn)( start );
      y[j] = objFunc(start, d_matchPairs, d_offset, d_sum, numPairs, pose1, poses_2, numPoses, angNom, angLim, uHigh, uLow, offset);
	  //printf("y[%d] = %lf\n", j, y[j]);
      *icount = *icount + 1;
      start[j] = x;
    }
    ...
    for ( ; ; ) {
      ...
      *ynewlo = y[0];
      ...
      // CHECK4
      //ystar = (*fn)( &pstar[0] );
      ystar = objFunc(&pstar[0], d_matchPairs, d_offset, d_sum, numPairs, pose1, poses_2, numPoses, angNom, angLim, uHigh, uLow, offset); 
      *icount = *icount + 1;
      ...
      if ( ystar < ylo ) {
        ...
        //y2star = (*fn)( &p2star[0] );
        y2star = objFunc(&p2star[0], d_matchPairs, d_offset, d_sum, numPairs, pose1, poses_2, numPoses, angNom, angLim, uHigh, uLow, offset); 
        *icount = *icount + 1;
	//  Check extension.
        if ( ystar < y2star ) {
	  ...
	  //  Contract the whole simplex.
          if ( y[ihi] < y2star ) {
            for (j = 0; j < nn; j++ ) {
              ...
              //y[j] = (*fn)( xmin );
              y[j] = objFunc(xmin, d_matchPairs, d_offset, d_sum, numPairs, pose1, poses_2, numPoses, angNom, angLim, uHigh, uLow, offset); 
              *icount = *icount + 1;
            }
            ylo = y[0];
            ilo = 0;
    ...
    *ifault = 0;
    ...
    for (i = 0; i < n; i++ ) {
      ...
      xmin[i] = xmin[i] + del;
      //z = (*fn)( xmin );
      z = objFunc(xmin, d_matchPairs, d_offset, d_sum, numPairs, pose1, poses_2, numPoses, angNom, angLim, uHigh, uLow, offset); 
      *icount = *icount + 1;
...
void nelminIndAngle ( int n, double start[], double xmin[], 
	      double *ynewlo, double reqmin, double step[], int konvge, int kcount, 
	      int *icount, int *numres, int *ifault,
	      int numPoses, int numPairs, double *d_matchPairs, double *d_offset, double *d_sum, double *pose1, double *poses_2,
	      double angNom, double angLim, double uHigh, double uLow
	      )
{
  ...
  *numres = 0;
  ...
  for ( ; ; ) {
    ...
    y[n] = objFuncIndAngle(start, d_matchPairs, d_offset, d_sum, numPairs, pose1, poses_2, numPoses, angNom, angLim, uHigh, uLow, offset); 
	//printf("y[%d] = %lf\n", n, y[n]);
    ...
    *icount = *icount + 1;
	...
	p[i+j*n] = start[i];
      ...
      //y[j] = (*fn)( start );
      y[j] = objFuncIndAngle(start, d_matchPairs, d_offset, d_sum, numPairs, pose1, poses_2, numPoses, angNom, angLim, uHigh, uLow, offset);
	  //printf("y[%d] = %lf\n", j, y[j]);
      *icount = *icount + 1;
      start[j] = x;
    }
    ...
    for ( ; ; ) {
      ...
      *ynewlo = y[0];
      ...
      // CHECK4
      //ystar = (*fn)( &pstar[0] );
      ystar = objFuncIndAngle(&pstar[0], d_matchPairs, d_offset, d_sum, numPairs, pose1, poses_2, numPoses, angNom, angLim, uHigh, uLow, offset); 
      *icount = *icount + 1;
      //  Successful reflection, so extension.
      if ( ystar < ylo ) {
        for (i = 0; i < n; i++ ) {
        ...
        //y2star = (*fn)( &p2star[0] );
        y2star = objFuncIndAngle(&p2star[0], d_matchPairs, d_offset, d_sum, numPairs, pose1, poses_2, numPoses, angNom, angLim, uHigh, uLow, offset); 
        *icount = *icount + 1;
	//  Check extension.
        if ( ystar < y2star ) {
          for (i = 0; i < n; i++ ) { p[i+ihi*n] = pstar[i]; }
	  ...
	  //  Contract the whole simplex.
          if ( y[ihi] < y2star ) {
            for (j = 0; j < nn; j++ ) {
              ...
              //y[j] = (*fn)( xmin );
              y[j] = objFuncIndAngle(xmin, d_matchPairs, d_offset, d_sum, numPairs, pose1, poses_2, numPoses, angNom, angLim, uHigh, uLow, offset); 
              *icount = *icount + 1;
            }
            ylo = y[0];
            ilo = 0;
    ...
    *ifault = 0;
    ...
    for (i = 0; i < n; i++ ) {
      ...
      xmin[i] = xmin[i] + del;
      //z = (*fn)( xmin );
      z = objFuncIndAngle(xmin, d_matchPairs, d_offset, d_sum, numPairs, pose1, poses_2, numPoses, angNom, angLim, uHigh, uLow, offset); 
      *icount = *icount + 1;
      if ( z < *ynewlo ) { *ifault = 2; break; }
      xmin[i] = xmin[i] - del - del;
      //z = (*fn)( xmin );
